...
async function getPostHash(hashtag) {
    return connectionDB.query(`SELECT posts.id, posts."user-id", posts.date, posts.text, posts.url FROM posts JOIN "posts-hashtags" ON posts.id = "posts-hashtags"."post-id" JOIN hashtags ON "posts-hashtags"."hashtag-id" = hashtags.id WHERE hashtags.name = $1 ORDER BY posts.date DESC;`, [hashtag])
}
